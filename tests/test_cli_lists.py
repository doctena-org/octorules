...
class TestListsValidate:
    """Tests for lists validation in cmd_validate."""

    def test_validate_lists_ok(self, tmp_path, caplog):
        ...
        assert result == 0
        assert "list:blocked_ips" in caplog.text
        assert "OK" in caplog.text
        assert "1 item(s)" in caplog.text

    def test_validate_lists_missing_name(self, tmp_path, caplog):
        rules_dir = tmp_path / "rules"
        rules_dir.mkdir()
        (rules_dir / "example.com.yaml").write_text(
            "lists:\n- kind: ip\n  items:\n  - ip: '1.2.3.4'\n"
    ...
    def test_validate_lists_invalid_kind(self, tmp_path, caplog):
        rules_dir = tmp_path / "rules"
        rules_dir.mkdir()
        (rules_dir / "example.com.yaml").write_text(
            "lists:\n- name: my_list\n  kind: invalid_kind\n  items: []\n"
    ...
    def test_validate_lists_no_unknown_phase_warning(self, tmp_path, caplog):
        """lists key should not trigger unknown phase warning."""
        rules_dir = tmp_path / "rules"
        rules_dir.mkdir()
        (rules_dir / "example.com.yaml").write_text(
            "redirect_rules:\n"
    ...
    def test_validate_lists_and_phases_together(self, tmp_path, caplog):
        """Both phase rules and lists should be validated."""
        rules_dir = tmp_path / "rules"
        rules_dir.mkdir()
        (rules_dir / "example.com.yaml").write_text(
            "redirect_rules:\n"
        ...
        with caplog.at_level(logging.INFO, logger="octorules"):
            result = cmd_validate(config, None)
        assert result == 0
        assert "redirect_rules: OK" in caplog.text
        assert "list:blocked_ips: OK" in caplog.text
...
class TestListsSync:
    ...
    def test_sync_creates_new_list(self, mock_provider_cls, tmp_path, caplog):
        ...
        assert result == 0
        provider.create_list.assert_called_once()
        call_args = provider.create_list.call_args
    ...
    def test_sync_deletes_removed_list(self, mock_provider_cls, tmp_path, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="octorules"):
            result = cmd_sync(config, None, scope_filter="account")
        assert result == 0
        provider.delete_list.assert_called_once_with(
    ...
    def test_sync_no_list_changes_skips_apply(self, mock_provider_cls, tmp_path):
        ...
        result = cmd_sync(config, None, scope_filter="account")
        assert result == 0
        provider.create_list.assert_not_called()
